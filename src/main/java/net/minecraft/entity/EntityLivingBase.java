        ...
        if (this.isServerWorld())
        {
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
            {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
                {
                    ...
                    //ncp se base sur ça mais cest vrm pas précis
                    //en gros tu fais un bhop legit mais tu increase les valeurs
                    //En tt cas sur les nouvelles versions de ncp
                    //mais pour funcraft l'ac est complètement débile
                    //dcp tu peux increase la valeur onground a chaque fois que tu jump
                    // cest pour ça tu va fast
                    //avec le bhop de liquidbounce
                    // ton exploit pour join en cracké c un spoofer az?
                    //nn en gros dans le launcher genre très vieux de 2016
                    //tu avais u ntruc qui générais des ip crack de funcraft a partir du pseudo
                    //dcp j'ai reverse le truc et j'ai fais ma propre implémentation
                    // comment t si smart mec
                    //omggg !
                    //d'ailleurs j'ai réussi a draw un rect de 100x100 sur az launcher
                    //avec des hooks cpp !!!!! bruhh
                    //oe faudrait je fasse spoofer mais grv la flemme surtout que leur jvm elle prends que des classes encryptées
//bon dcp tu veux que je te fasse quoi mmmmh un truc genre nametags car jsp du tout comment faire ;-;
                    //oula je suis pas du tout bon en render hein bon ntm le trackpad de merde
                    //cmt ça
                    //parce que la friction est différente pour chaque ac mdrr bah pour fc alors meme si jpourrais pas le tester lol
                    //ben vzy
                    if (this.isOnLadder())
                    {
                        float f6 = 0.15F;
