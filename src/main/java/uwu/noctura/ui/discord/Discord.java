package uwu.noctura.ui.discord;
...
import uwu.noctura.utils.DiscordUtil;
...
public class Discord extends GuiScreen {
    ...
    public Category Category = uwu.noctura.module.Category.Combat;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)  {
        ...
        Gui.drawRect(guiX, guiY, guiX + width, guiY + height, new Color(54, 57, 63, 255).getRGB());
        Gui.drawRect(guiX, guiY, guiX + width, guiY + 14, new Color(32, 34, 37, 255).getRGB());
        ...
        if (currentModule == null) {
            ...
            Noctura.INSTANCE.getFontManager().getFont("auxy 15").drawString(renderDone ? "" : DiscordUtil.getDiscordName(), (float) (guiX + 90 - 53.5 - 3.2), guiY + 17 + 258 + 4 , -1);

            Noctura.INSTANCE.getFontManager().getFont("auxy 15").drawString("#" + DiscordUtil.getDiscordHash(), (float) (guiX + 90 - 53.5 - 3.2), guiY + 17 + 258 + 12, -1);
        }
        FontManager fm = Noctura.INSTANCE.getFontManager();
        Noctura.INSTANCE.getFontManager().getFont("Auxy 40").drawString("F" + "lauxy", guiX + 2, guiY + 2
                , new Color(163, 166, 170, 255).getRGB());
        crossX = guiX + width - fm.getFont("Auxy 40").getWidth("X") - 2;

        Noctura.INSTANCE.getFontManager().getFont("Auxy 40").drawString("X", crossX, guiY + 2
                , new Color(185, 187, 190, 255).getRGB());
        ...
        if (currentModule == null || currentModule.getSettings().isEmpty()) {
            float cateY = guiY + fixedYValue;
            for (Category m : Category.values()) {
                RenderUtil.drawCircle(guiX + 23.5f, cateY + 4, 12, isHovered(guiX + 11, cateY - 9, guiX + 35, cateY + 15, mouseX, mouseY)
                        ? new Color(88, 101, 242).getRGB() : new Color(122, 125, 135).getRGB()
                );
                if (m == Category) {

                    getFont().drawString(String.valueOf(m.name().charAt(0)), guiX + 20, cateY, new Color((int) ClickGUI.red.getValue(), (int) ClickGUI.green.getValue(), (int) ClickGUI.blue.getValue()).getRGB());
                    if (isHovered(guiX, guiY, guiX + width, guiY + 20, mouseX, mouseY) && Mouse.isButtonDown(0)) {
                        hy = cateY;
                    } else {
                        if (hy != cateY) {
                    ...
                    getFont().drawString(String.valueOf(m.name().charAt(0)), guiX + 20, cateY, -1);
                }
                cateY += 25;
            }
        }
        ...
        if (currentModule != null && !currentModule.getSettings().isEmpty()) {
            ...
            getFont().drawString(currentModule.getName(), guiX + 435 + moduleX + 80 - 79, guiY + fixedYValue + 3, -1);
            if (isHovered(guiX + 435 + moduleX, guiY + fixedYValue, guiX + 435 + moduleX + 16, guiY + fixedYValue + 16, mouseX, mouseY) && Mouse.isButtonDown(0)) {
                currentModule = null;
                timer.reset();
            }
        ...
        GL11.glEnable(GL11.GL_SCISSOR_TEST);
        GL11.glScissor(0, 2 * ((int) (sr.getScaledHeight_double() - (guiY + height))) + 80, (int) (sr.getScaledWidth_double() * 2), (int) ((height) * 2) - 160);
        ...
        if (isHovered(guiX + 100 + moduleX, guiY + 60, guiX + 425 + moduleX, guiY + height, mouseX, mouseY)) {
            if (currentDWheel < 0 && Math.abs(role) + 220 < (Noctura.INSTANCE.getModuleManager().getModules(Category).length * 40)) {
                role -= 32;
            }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        float categoryY = guiY + fixedYValue;
        if(mouseX >= crossX - 2 && mouseX <= crossX + Noctura.INSTANCE.getFontManager().getFont("Auxy 40").getWidth("X") + 2){
            if(!closed){
                close = true;
        ...
        for (Category m : Category.values()) {
            ...
            if (isHovered(guiX, categoryY - 8, guiX + 50, categoryY + getFont().getHeight("") + 8, mouseX, mouseY)) {
                ...
                Category = m;
                for (Module module : Noctura.INSTANCE.getModuleManager().modules) {
