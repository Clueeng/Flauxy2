...
import uwu.noctura.utils.InfoEntry;
        ...
        if(e instanceof EventRender2D){
            ...
            for (InfoEntry entry : infoEntries) {
                if (customFont.getValue()) {
                    getFont().drawStringWithShadow(entry.getFormattedText(), getMoveX(), getMoveY() + offsetY, Noctura.INSTANCE.moduleManager.getModule(ArrayList.class).stringColor);
                } else {
                    mc.fontRendererObj.drawStringWithShadow(entry.getFormattedText(), getMoveX(), getMoveY() + offsetY, Noctura.INSTANCE.moduleManager.getModule(ArrayList.class).stringColor);
                }
                offsetY += 12;
            }
        }
